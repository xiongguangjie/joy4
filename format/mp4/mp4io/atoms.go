		...
		case STCO, CO64:
			{
				atom := &ChunkOffset{}
				if _, err = atom.Unmarshal(b[n:n+size], offset+n); err != nil {
					err = parseErr(tag.String(), n+offset, err)
					return
				}
...
func (self *ChunkOffset) Unmarshal(b []byte, offset int) (n int, err error) {
	(&self.AtomPos).setPos(offset, len(b))
	self.Is64bit = Tag(pio.U32BE(b[4:])) == CO64
	n += 8
	if len(b) < n+1 {
		err = parseErr("Version", n+offset, err)
		return
	...
	_len_Entries = pio.U32BE(b[n:])
	n += 4
	self.Entries = make([]int64, _len_Entries)
	var entriesSize int
	if self.Is64bit {
		entriesSize = 8 * len(self.Entries)
	} else {
		entriesSize = 4 * len(self.Entries)
	}
	if len(b) < n+entriesSize {
		if self.Is64bit {
			err = parseErr("int64", n+offset, err)
		} else {
			err = parseErr("uint32", n+offset, err)
		}
		return
	...
	if self.Is64bit {
		for i := range self.Entries {
			self.Entries[i] = pio.I64BE(b[n:])
			n += 8
		}
	} else {
		for i := range self.Entries {
			self.Entries[i] = int64(pio.U32BE(b[n:]))
			n += 4
		}
	}
	return
